...
const Header = () => {
  ...
  const {  searchQuery, setSearchQuery,getFilteredNote} = useNote();
  ...
  return (
    <div className="py-5 dark:text-slate-400 px-8 flex justify-between items-center ">
      <div className="w-80 relative flex-1 ">
        ...
        />
        <BiSearch onClick={handleSearch} className="absolute top-4 text-xl right-4 text-slate-400 hover:cursor-pointer" />
      </div>
      ...
      <div className="flex-1 flex justify-end items-center gap-6">
        ...
        <button className=" w-36 text-white py-3 rounded-lg bg-violet-600 hover:bg-violet-700 dark:bg-violet-800" onClick={openModal}>
          Add new task
        </button>
      </div>
    </div>
